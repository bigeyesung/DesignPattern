...
class Mediator:
    ...
    def SetTasks(self,task):
        if self.tasks.get(task.attri["GUID"]) is None:
            self.tasks[task.attri["GUID"]] = task
        print(task.GetElementGUID())
        print(task.attri["GUID"])
        self.SetLinks(task.attri["GUID"], task.GetElementGUID())
    ...
    def GetTask(self, id):
        ...
        return self.tasks[taskid].PrintGUID()
...
class Element:
    ...
    def do(self):
        print("element guid: {} , taskid: {} ".format(self.guid, self.taskid))
